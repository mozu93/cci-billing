...
def test_dialog_default_positional_mapping(qtbot, memory_db):
    from app.ui.member_import import MemberImportDialog
    dlg = MemberImportDialog()
    qtbot.addWidget(dlg)
    dlg._set_raw_rows([["A-001", "○○商事", "", "田中"]])
    mapping = dlg._current_mapping()
    assert mapping["member_number"] == 0
    assert mapping["organization_name"] == 1
...
def test_dialog_header_checkbox_skips_first_row(qtbot, memory_db):
    from app.ui.member_import import MemberImportDialog
    dlg = MemberImportDialog()
    qtbot.addWidget(dlg)
    dlg._set_raw_rows([["事業所名", "会員番号"], ["○○商事", "A-001"]])

    # 見出しOFF：1行目もデータ扱い（事業所名="事業所名" が入ってしまう）
    assert len(dlg._mapped_rows()) == 2

    # 見出しON：見出し名から自動割り当てし、1行目を除外
    dlg._header_chk.setChecked(True)
    rows = dlg._mapped_rows()
    assert len(rows) == 1
    assert rows[0]["organization_name"] == "○○商事"
    assert rows[0]["member_number"] == "A-001"
