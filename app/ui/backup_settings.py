...
class BackupSettingsWidget(QWidget):
    ...
    def _build(self):
        ...
        btn_row = QHBoxLayout()
        btn_row.addWidget(QPushButton("設定を保存")).clicked.connect(self._save_config)
        btn_row.addWidget(QPushButton("今すぐバックアップ")).clicked.connect(self._backup_now)
        btn_row.addWidget(QPushButton("一覧を更新")).clicked.connect(self._load_list)
        btn_row.addStretch()
        ...
        btn_row2.addWidget(QPushButton("選択したバックアップを復元")).clicked.connect(self._restore))
        btn_row2.addStretch()
